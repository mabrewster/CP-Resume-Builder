...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .column {
      ...
      flex: 1 1 420px;
      overflow: visible; /* IMPORTANT: allows full snapshot */
    }
    ...
    .resume-section h3 { margin-top: 0; font-size: 1.05rem; }
    .resume-item {
      margin: 6px 0;
      padding: 8px 10px;
      background: #f4f7fb;
    ...
    /* Cleaner look for the exported PDF snapshot only */
  ...
  <!-- ✅ Correct bundle with integrity (html2canvas + jsPDF included) -->
  <script
...
<body>
  ...
  <div class="directions no-print">
    <strong>Directions:</strong> Drag skill cards from the left into the resume sections on the right.
    Add your own cards if you wish. When finished, click <em>Print to PDF</em> and upload to Edio.
  </div>
  ...
  <script>
    ...
    async function downloadPDF() {
      ...
      // If the bundle can't load (e.g., CDN blocked), fall back to print dialog
      const hasH2C = typeof window.html2canvas !== 'undefined';
      const hasJsPDF = window.jspdf && window.jspdf.jsPDF;
      if (!hasH2C || !hasJsPDF) {
        try { window.print(); return; }
        catch { alert('PDF tools are blocked. Try a different network/browser.'); return; }
      }
      ...
      try {
        ...
        try { window.print(); }
        catch { alert('Sorry—PDF could not be generated. Try reloading or a different browser.'); }
      } finally {
